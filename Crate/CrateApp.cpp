...
//	texTable.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 0);
	texTable.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 2, 0); // 연습문제 9.3
...
//	auto woodCrateTex = mTextures["woodCrateTex"]->Resource;
	auto flareTex = mTextures["flareTex"]->Resource; // 연습문제 9.3
	auto flarealphaTex = mTextures["flarealphaTex"]->Resource; // 연습문제 9.3
 
	D3D12_SHADER_RESOURCE_VIEW_DESC srvDesc = {};
	srvDesc.Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;
	//srvDesc.Format = woodCrateTex->GetDesc().Format;
	srvDesc.Format = flareTex->GetDesc().Format; // 연습문제 9.3
	srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2D;
	srvDesc.Texture2D.MostDetailedMip = 0;
	//srvDesc.Texture2D.MipLevels = woodCrateTex->GetDesc().MipLevels;
	srvDesc.Texture2D.MipLevels = flareTex->GetDesc().MipLevels; // 연습문제 9.3
	srvDesc.Texture2D.ResourceMinLODClamp = 0.0f;

	//md3dDevice->CreateShaderResourceView(woodCrateTex.Get(), &srvDesc, hDescriptor);
	md3dDevice->CreateShaderResourceView(flareTex.Get(), &srvDesc, hDescriptor); // 연습문제 9.3

	hDescriptor.Offset(1, mCbvSrvDescriptorSize); // 연습문제 9.3
	srvDesc.Format = flarealphaTex->GetDesc().Format; // 연습문제 9.3
	md3dDevice->CreateShaderResourceView(flarealphaTex.Get(), &srvDesc, hDescriptor); // 연습문제 9.3
}
...
void CrateApp::BuildMaterials()
{
	...
	auto flarealpha = std::make_unique<Material>();
	flarealpha->Name = "flarealpha";
	flarealpha->MatCBIndex = 1;
	flarealpha->DiffuseSrvHeapIndex = 0;
	flarealpha->DiffuseAlbedo = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
	flarealpha->FresnelR0 = XMFLOAT3(0.05f, 0.05f, 0.05f);
	flarealpha->Roughness = 0.2f;

	mMaterials["flarealpha"] = std::move(flarealpha);
}
...
void CrateApp::BuildRenderItems()
{
	...
	boxRitem->Mat = mMaterials["flare"].get();	// 연습문제 9.3
	boxRitem->Mat = mMaterials["flarealpha"].get();	// 연습문제 9.3
...
std::array<const CD3DX12_STATIC_SAMPLER_DESC, 8> CrateApp::GetStaticSamplers()
{
	...
	const CD3DX12_STATIC_SAMPLER_DESC linearBorder(
		...
		D3D12_TEXTURE_ADDRESS_MODE_BORDER);     // 연습문제 9.1
	...
	const CD3DX12_STATIC_SAMPLER_DESC linearMirror(
		...
		D3D12_TEXTURE_ADDRESS_MODE_MIRROR); // 연습문제 9.1

	return { 
		pointWrap, pointClamp,
		linearWrap, linearClamp, 
		anisotropicWrap, anisotropicClamp, linearBorder, linearMirror };
}
