...
private:
	...
	const int NumDataElements = 64;	// 연습문제 13.1
...
void VecAddCSApp::DoComputeWork()
{
	...
	float* mappedData = nullptr;	// 연습문제 13.1
	...
	{
		fout << "(" << mappedData[i] << ")" << std::endl;	// 연습문제 13.1
	}
...
void VecAddCSApp::BuildBuffers()
{
	...
	for(int i = 0; i < NumDataElements; ++i)
	{
		while (1)
		{
			tempX = rand() / (float)RAND_MAX * 9 + 1;
			tempY = rand() / (float)RAND_MAX * 9 + 1;
			tempZ = rand() / (float)RAND_MAX * 9 + 1;

			if ((1 <= sqrt(tempX * tempX + tempY * tempY + tempZ * tempZ)) && (sqrt(tempX * tempX + tempY * tempY + tempZ * tempZ) <= 10))
			{
				dataA[i].v1 = XMFLOAT3(tempX, tempY, tempZ);
				break;
			}
		}
	}
	...
	ThrowIfFailed(md3dDevice->CreateCommittedResource(
		...
		&CD3DX12_RESOURCE_DESC::Buffer(dataA.size() * sizeof(float), D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS),
	...
	ThrowIfFailed(md3dDevice->CreateCommittedResource(
		...
		D3D12_HEAP_FLAG_NONE,
		&CD3DX12_RESOURCE_DESC::Buffer(dataA.size() * sizeof(float)),
